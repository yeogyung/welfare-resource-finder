(function (root, factory) {
  ...
  function scoreResource(item, query, originalQuery = query, category = "all") {
    ...
    if (axis === "activity") {
      ...
      if (/문화|여가|모임|사회참여|평생교육|노래|독서|공연|전시|티켓|봉사/.test(text)) {
    ...
    if (/광진|복지관|스마트폰|교육|모임/.test(original) && item.source === "gwangjin") add(12, "광진구 실증 DB 우선");
  ...
  function buildLead(query, results) {
    ...
    if (axis === "activity") return `좋아요. "${query}"와 관련된 여가·문화·배움 자원을 먼저 찾아봤어요.`;
    if (axis === "life") return `말씀해 주셔서 고마워요. "${query}"와 관련된 생활형 도움 자원을 찾아봤어요.`;
  ...
  function inferAxis(text) {
    ...
    if (/배우|교육|공연|전시|문화|여가|모임|독서|글쓰기|노래|합창|봉사|참여|활동|스마트폰|컴퓨터|키오스크|경로당|취미|나들이/.test(value)) {
      return "activity";
    }
