...
개발물에 넣어야 하는 기준 DB는 `public/data/welfare-resources.json`이다. 이 파일은 팀이 정리한 원본 엑셀 4종을 `scripts/build_resources.py`로 통합 변환한 앱용 데이터다.
...
| 구분 | 위치 | 용도 | 사용자 노출 |
|---|---|---|---|
| 원본 DB | 배경지식/00_원본자료/다운로드_첨부자료 | 팀원이 수집·라벨링한 엑셀 원본 | 노출 안 함 |
