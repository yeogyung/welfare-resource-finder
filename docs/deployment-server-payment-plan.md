...
- 체리카드 해외결제가 불가하므로 OpenAI, Claude 등 해외 가맹점 직접 결제는 제외한다.
- 원화로 표시되더라도 해외 가맹점이면 결제가 막힐 수 있으므로, 실증용 AI/음성 기능은 국내 결제 가능한 네이버클라우드 중심으로 설계한다.
...
| P1 | 네이버클라우드 계정 결제 확인 | 체리카드 국내 결제 가능성 확보 | 가능 확인됨 |
...
- 해외 AI API 직접 결제: 체리카드 해외결제 제한 때문에 제외
...
> 체리카드의 해외결제 제한을 고려하여 OpenAI 등 해외 AI API 직접 결제 방식은 제외하고, 국내 결제가 가능한 네이버클라우드 기반 STT/TTS/LLM 연동 구조를 설계하였다. 1차 배포는 Vercel과 GitHub를 활용한 정적 웹 MVP로 진행하고, 2차 실증 단계에서 NAVER Cloud CLOVA Speech/Voice 및 CLOVA Studio를 서버리스 API로 연동하는 방식으로 확장 계획을 수립하였다.
